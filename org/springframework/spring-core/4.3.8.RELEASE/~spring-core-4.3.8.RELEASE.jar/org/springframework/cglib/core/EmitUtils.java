...
// Source File Name:   EmitUtils.java
...
//			ClassEmitter, CodeEmitter, Signature, TypeUtils, 
//			ProcessArrayCallback, CodeGenerationException, CollectionUtils, Constants, 
//			ObjectSwitchCallback, HashCodeCustomizer, Customizer, MethodInfo, 
//			ClassInfo, Block, Transformer, ProcessSwitchCallback
...
public class EmitUtils
{
	private static interface ParameterTyper
	{
		...
		public abstract Type[] getParameterTypes(MethodInfo methodinfo);
	}
	...
	public static void string_switch(CodeEmitter e, String strings[], int switchStyle, ObjectSwitchCallback callback)
	{
		try
		{
			...
			default:
				throw new IllegalArgumentException((new StringBuilder()).append("unknown switch style ").append(switchStyle).toString());
			}
		}
		...
		throws Exception
	{
		...
		e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback(buckets, e, callback, def, end) {

			final Map val$buckets;
			final CodeEmitter val$e;
			final ObjectSwitchCallback val$callback;
			final Label val$def;
			final Label val$end;
		...
		throws Exception
	{
		...
		Map buckets = CollectionUtils.bucket(strings, new Transformer(index) {
		...
		e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback(buckets, index, len, e, callback, end, def) {
		...
		throws Exception
	{
		...
		e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback(buckets, skipEquals, e, callback, end, def) {
	...
	private static void load_class_helper(CodeEmitter e, Type type)
	{
		...
		} else
		{
			...
			String fieldName = (new StringBuilder()).append("CGLIB$load_class$").append(TypeUtils.escapeType(typeName)).toString();
	...
	{
		e.push(array.length);
		e.newarray(Type.getType(remapComponentType(((Object) (array)).getClass().getComponentType())));
	...
	{
		if (componentType.equals(org/springframework/asm/Type))
	...
	public static void push_object(CodeEmitter e, Object obj)
	{
		...
		} else
		{
			...
			{
				throw new IllegalArgumentException((new StringBuilder()).append("unknown type: ").append(obj.getClass()).toString());
			}
		}
	}

	/**
	 * @deprecated Method hash_code is deprecated
	 */
	...
	private static void hash_array(CodeEmitter e, Type type, int multiplier, CustomizerRegistry registry)
	{
		...
		e.dup();
		e.ifnull(skip);
		process_array(e, type, new ProcessArrayCallback(e, multiplier, registry) {

			final CodeEmitter val$e;
			final int val$multiplier;
			final CustomizerRegistry val$registry;
	...
	private static void hash_object(CodeEmitter e, Type type, CustomizerRegistry registry)
	{
		...
		Iterator iterator = registry.get(org/springframework/cglib/core/HashCodeCustomizer).iterator();
		do
		{
			if (!iterator.hasNext())
				break;
			HashCodeCustomizer customizer = (HashCodeCustomizer)iterator.next();
		...
		if (!customHashCode)
		{
			...
			for (Iterator iterator1 = registry.get(org/springframework/cglib/core/Customizer).iterator(); iterator1.hasNext(); customizer.customize(e, type))
				customizer = (Customizer)iterator1.next();
	...
	/**
	 * @deprecated Method not_equals is deprecated
	 */
	...
	{
		(new ProcessArrayCallback(e, notEquals, registry) {

			final CodeEmitter val$e;
			final Label val$notEquals;
			final CustomizerRegistry val$registry;
	...
	private static void not_equals_helper(CodeEmitter e, Type type, Label notEquals, CustomizerRegistry registry, ProcessArrayCallback callback)
	{
		...
		} else
		{
			...
			{
				List customizers = registry.get(org/springframework/cglib/core/Customizer);
				if (!customizers.isEmpty())
				{
					Customizer customizer;
					for (Iterator iterator = customizers.iterator(); iterator.hasNext(); customizer.customize(e, type))
						customizer = (Customizer)iterator.next();

					e.swap();
					Customizer customizer;
					for (Iterator iterator1 = customizers.iterator(); iterator1.hasNext(); customizer.customize(e, type))
						customizer = (Customizer)iterator1.next();
	...
	/**
	 * @deprecated Method append_string is deprecated
	 */
	...
	{
		ArrayDelimiters d = delims == null ? DEFAULT_DELIMITERS : delims;
		ProcessArrayCallback callback = new ProcessArrayCallback(e, d, registry) {

			final CodeEmitter val$e;
			final ArrayDelimiters val$d;
			final CustomizerRegistry val$registry;
	...
	private static void append_string_helper(CodeEmitter e, Type type, ArrayDelimiters delims, CustomizerRegistry registry, ProcessArrayCallback callback)
	{
		...
		} else
		{
			...
			e.ifnull(skip);
			Customizer customizer;
			for (Iterator iterator = registry.get(org/springframework/cglib/core/Customizer).iterator(); iterator.hasNext(); customizer.customize(e, type))
				customizer = (Customizer)iterator.next();
	...
	private static void member_switch_helper(CodeEmitter e, List members, ObjectSwitchCallback callback, boolean useName)
	{
		try
		{
			...
			ParameterTyper cached = new ParameterTyper(cache) {
			...
			if (useName)
			{
				...
				string_switch(e, names, 1, new ObjectSwitchCallback(e, buckets, callback, cached, def, end) {

					final CodeEmitter val$e;
					final Map val$buckets;
					final ObjectSwitchCallback val$callback;
					final ParameterTyper val$cached;
					final Label val$def;
					final Label val$end;
	...
	private static void member_helper_size(CodeEmitter e, List members, ObjectSwitchCallback callback, ParameterTyper typer, Label def, Label end)
		throws Exception
	{
		Map buckets = CollectionUtils.bucket(members, new Transformer(typer) {

			final ParameterTyper val$typer;
			...
			{
				typer = parametertyper;
				Object();
			}
		});
		e.dup();
		e.arraylength();
		e.process_switch(getSwitchKeys(buckets), new ProcessSwitchCallback(buckets, e, callback, typer, def, end) {
		...
		throws Exception
	{
		...
		} else
		{
			...
			{
				int j = i;
				Map test = CollectionUtils.bucket(members, new Transformer(typer, j) {
			...
			} else
			{
				...
				String names[] = (String[])(String[])buckets.keySet().toArray(new String[buckets.size()]);
				string_switch(e, names, 1, new ObjectSwitchCallback(e, fbuckets, callback, typer, def, end, checked) {

					final CodeEmitter val$e;
					final Map val$fbuckets;
					final ObjectSwitchCallback val$callback;
					final ParameterTyper val$typer;
					final Label val$def;
					final Label val$end;
					final BitSet val$checked;
	...
	public static void add_properties(ClassEmitter ce, String names[], Type types[])
	{
		...
		{
			String fieldName = (new StringBuilder()).append("$cglib_prop_").append(names[i]).toString();
	...
	public static void add_property(ClassEmitter ce, String name, Type type, String fieldName)
	{
		...
		CodeEmitter e = ce.begin_method(1, new Signature((new StringBuilder()).append("get").append(property).toString(), type, Constants.TYPES_EMPTY), null);
		e.load_this();
		e.getfield(fieldName);
		e.return_value();
		e.end_method();
		e = ce.begin_method(1, new Signature((new StringBuilder()).append("set").append(property).toString(), Type.VOID_TYPE, new Type[] {
			type
		}), null);
		e.load_this();
		e.load_arg(0);
	...
	{
		Set set = ((Set) (exceptions != null ? ((Set) (new HashSet(Arrays.asList(exceptions)))) : Collections.EMPTY_SET));

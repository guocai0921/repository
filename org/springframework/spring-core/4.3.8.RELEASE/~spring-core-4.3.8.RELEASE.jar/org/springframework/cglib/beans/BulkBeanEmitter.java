...
// Source File Name:   BulkBeanEmitter.java
...
class BulkBeanEmitter extends ClassEmitter
{
	...
	private void generateGet(Class target, Method getters[])
	{
		...
		if (getters.length >= 0)
		{
			...
			org.springframework.cglib.core.Local bean = e.make_local();
			e.store_local(bean);
	...
	private void generateSet(Class target, Method setters[])
	{
		...
		if (setters.length > 0)
		{
			org.springframework.cglib.core.Local index = e.make_local(Type.INT_TYPE);
			e.push(0);
			e.store_local(index);
			e.load_arg(0);
			e.checkcast(Type.getType(target));
			...
			for (i = 0; i < types.length; i++)
			{
				if (getters[i] != null)
				{
					...
					if (method.getReturnType() != types[i])
						throw new BulkBeanException((new StringBuilder()).append("Specified type ").append(types[i]).append(" does not match declared type ").append(method.getReturnType()).toString(), i);
				...
				if (setters[i] != null)
				{
					Method method = ReflectUtils.findDeclaredMethod(target, setters[i], new Class[] {
						types[i]
					});
					if (Modifier.isPrivate(method.getModifiers()))
						throw new BulkBeanException("Property is private", i);

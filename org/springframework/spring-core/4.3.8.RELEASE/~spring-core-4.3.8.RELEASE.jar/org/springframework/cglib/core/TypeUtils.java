...
//			Signature, Constants, MethodInfo, CollectionUtils
...
public class TypeUtils
{
	...
	{
		return Type.getType((new StringBuilder()).append("L").append(className.replace('.', '/')).append(";").toString());
	}
	...
	public static String getClassName(Type type)
	{
		...
		if (isArray(type))
			return (new StringBuilder()).append(getClassName(getComponentType(type))).append("[]").toString();
		else
	...
	{
		return Type.getType((new StringBuilder()).append("L").append(name).append(";").toString());
	}
	...
	public static Signature parseSignature(String s)
	{
		...
		sb.append('(');
		for (Iterator it = parseTypes(s, lparen + 1, rparen).iterator(); it.hasNext(); sb.append(it.next()));
		sb.append(')');
	...
	{
		return parseSignature((new StringBuilder()).append("void <init>(").append(sig).append(")").toString());
	}
	...
	private static String map(String type)
	{
		...
		if (t != null)
			return t;
		if (type.indexOf('.') < 0)
			return map((new StringBuilder()).append("java.lang.").append(type).toString());
		StringBuffer sb = new StringBuffer();
		for (int index = 0; (index = type.indexOf("[]", index) + 1) > 0;)
			sb.append('[');
	...
	{
		if (!isArray(type))
			throw new IllegalArgumentException((new StringBuilder()).append("Type ").append(type).append(" is not an array").toString());
		else

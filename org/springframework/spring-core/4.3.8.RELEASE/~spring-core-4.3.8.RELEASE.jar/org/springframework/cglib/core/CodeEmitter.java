...
// Source File Name:   CodeEmitter.java
...
//			LocalVariablesSorter, ClassEmitter, Signature, Block, 
//			TypeUtils, Constants, Local, ProcessSwitchCallback, 
//			CodeGenerationException, MethodInfo, ClassInfo
...
public class CodeEmitter extends LocalVariablesSorter
{
	...
	public void if_cmp(Type type, int mode, Label label)
	{
		...
		case 10: // '\n'
			...
			throw new IllegalArgumentException((new StringBuilder()).append("Bad comparison for type ").append(type).toString());
	...
	{
		if (sig.getName().equals("<init>") && opcode != 182)
			if (opcode != 184);
		mv.visitMethodInsn(opcode, type.getInternalName(), sig.getName(), sig.getDescriptor(), opcode == 185);
	}
	...
	public void box(Type type)
	{
		if (TypeUtils.isPrimitive(type))
			...
			} else
			{
				...
				invoke_constructor(boxed, new Signature("<init>", Type.VOID_TYPE, new Type[] {
					type
				}));
			}
	}
